...
zeroyt7.on("group-participants-update", async (anu) => {
    ...
    try {
      ...
      if (anu.action == "add" && mem.includes(zeroyt7.user.jid)) {
        zeroyt7.sendMessage(anu.jid, "Hallo Saya Bot WhatsApp Jika Mau Gunain Ketik #menu", "conversation")
      }
      if (!isWelkom) return
      if (anu.action == "add" && !mem.includes(zeroyt7.user.jid)) {
        ...
        wel = `Hallo @${anu_user} \nWelcome Gc ${mdata.subject} \nJangan Lupa Patuhi Peraturan Grup`
        ...
        sendButImage(mdata.id, wel, "Hallo Member Baru", buff, but)
      }
      if (!isWelkom) return
      if (anu.action == "remove" && !mem.includes(zeroyt7.user.jid)) {
        ...
        sendButImage(mdata.id, out, "Selamat Tinggal Member Kang Out", buff, but)
      }
